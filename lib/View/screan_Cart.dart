...
class _screan_CartState extends State<screan_Cart> {
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(textDirection: TextDirection.rtl,
      child: Consumer<State_Cart>(builder: (context, cart , child) => Scaffold(
        backgroundColor: Colors.white,
          ...
          appBar: AppBar(
            elevation: 0,
            centerTitle: true,
            title: Text(" سلة المشتريات ",style: TextStyle(fontSize:fontsize),),
            actions: <Widget>[
              FutureBuilder(
                future: apiGetCart(macAddress: _MacAddress),
                  builder: (context, snapshot) {
                    return Button_CartAppBar(context: context , cartLength: getCart.length.toString() ?? "", cart: cart ,onTap: (){});
                  },
              ),
        ...
        body:FutureBuilder(
          ...
          builder: (context, snapshot) {
            if(snapshot.connectionState == ConnectionState.done){
              ...
              EmptyCart(fontsize: fontsize ,onTap: (){ print("object"); }) :
                  ...
                  slivers: <Widget>[
                    ...
                    SliverList(delegate: SliverChildListDelegate([
                      ...
                      new Container(
                        ...
                        child: Column(
                          children: <Widget>[
                            buildRow(context: context,name: "المجموع" ,price: "${getDataCart?.totalPrice ?? ""}" ,color: Colors.grey[500]),
                            buildRow(context: context,name: "رسوم التوصيل" ,price: "${getDataCart?.totaShippingCost ?? ""}" ,color: Colors.grey[500]),
                            buildRow(context: context,name: "اجمالي افاتورة" ,price: "${getDataCart?.total ?? ""}"),
                          ],
                        ),
                      ),
